...
# rollout/vendor/bridge -> repo root
_REPO_ROOT = Path(__file__).resolve().parents[3]
_LOCAL_ASSET_ROOT = Path(__file__).resolve().parents[1] / "assets"
...
def create_g1_robot_cfg(
    *,
    ...
    if forward_speed is not None:
        move_to_point.forward_speed = float(forward_speed)
    if rotation_speed is not None:
        move_to_point.rotation_speed = float(rotation_speed)
    if rotate_speed is not None:
        rotate.rotation_speed = float(rotate_speed)
    elif rotation_speed is not None:
        rotate.rotation_speed = float(rotation_speed)
    if move_to_point_threshold is not None:
        move_to_point.threshold = float(move_to_point_threshold)
...
def create_franka_robot_cfg(
    *,
    ...
    controllers: list[Any] = [arm_ik, gripper]
    if include_arm_joint_controller:
        controllers.append(
            JointControllerCfg(
...
def create_franka_rmpflow_robot_cfg(
    *,
    position: Sequence[float] = (0.0, 0.0, 0.0),
    usd_path: str | None = None,
) -> Any:
    """Merom Franka: native Isaac ``omni.isaac.franka.Franka`` + RMPFlow + gripper.

    Same robot class / controller stack as ``franka_manipulation_mocap_teleop.py``
    (``MocapControlledFrankaRobot``), without mocap teleop wrapper.
    """
    from internutopia_extension.configs.controllers import GripperControllerCfg, RMPFlowControllerCfg
    from internutopia_extension.configs.robots.mocap_controlled_franka import (
        MocapControlledFrankaRobotCfg,
    )

    rmpflow = RMPFlowControllerCfg(
        name="rmpflow_controller",
        robot_description_path=_local_asset_path("robots/franka/rmpflow/robot_descriptor.yaml"),
        robot_urdf_path=_local_asset_path("robots/franka/lula_franka_gen.urdf"),
        rmpflow_config_path=_local_asset_path("robots/franka/franka_rmpflow_common.yaml"),
        end_effector_frame_name="right_gripper",
    )
    gripper = GripperControllerCfg(name="gripper_controller")
    robot_usd = usd_path or _local_asset_path("robots/franka/franka.usd")
    return MocapControlledFrankaRobotCfg(
        name="franka",
        type="MocapControlledFrankaRobot",
        prim_path="/franka",
        position=tuple(position),
        usd_path=robot_usd,
        controllers=[rmpflow, gripper],
        sensors=[],
    )
