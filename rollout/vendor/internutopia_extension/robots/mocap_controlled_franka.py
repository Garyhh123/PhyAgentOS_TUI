...
class MocapControlledFrankaRobot(BaseRobot):
    ...
    @staticmethod
    def _world_pose(body: Any) -> tuple[np.ndarray, np.ndarray]:
        if body is None:
            return np.array([0.0, 0.0, 0.0]), np.array([1.0, 0.0, 0.0, 0.0])
        if hasattr(body, "get_world_pose"):
            return body.get_world_pose()
        if hasattr(body, "get_pose"):
            return body.get_pose()
        raise AttributeError(f"{type(body).__name__} has no get_world_pose/get_pose")

    def get_obs(self) -> OrderedDict:
        position, orientation = self._world_pose(self.articulation)
        ...
        eef_body = getattr(self.articulation, "end_effector", None)
        if eef_body is None:
            eef_path = f"{self.config.prim_path}/panda_hand"
            eef_body = getattr(self, "_rigid_body_map", {}).get(eef_path)
        eef_pos, eef_ori = self._world_pose(eef_body)
        obs['eef_position'] = eef_pos
        obs['eef_orientation'] = eef_ori
