...
The new SessionRunner-based architecture routes through *run_builtin_loop*
which uses TargetSessionHandle for all target interactions.  The legacy
``run()`` entrypoint is kept for backward compatibility with older
non-SessionRunner callers.
"""
...
class MinecraftSkillRuntime(BuiltinSkillRuntime):
    ...
    def run_builtin_loop(
        ...
        if task_plan is not None:
            ...
            result = self._run_flat_handle(queries, session, target_handle,
                                           adapter_plan)
    ...
    def _run_task_plan_handle(
        ...
        for sg in plan.subgoals:
            ...
            verifier = TaskVerifier(handle._target, raw_obs.data)
            ...
            for task in sg.tasks:
                ...
                for attempt in range(task.max_retries):
                    ...
                    for action_spec in task.actions:
                        ...
                        num_steps += 1
                        transition = chunk_result

                        if action_spec.type == "move" and action_spec.params.get("absolute"):
                            if handle._pending_raw_observation is None:
                                time.sleep(0.5)
                            raw_obs = handle.observe()
    ...
    def _run_flat_handle(
        ...
        for step_idx in range(session.execution.max_steps):
            ...
            action = _pick_action(action_plan, step_idx, handle)
            chunk_result = handle.action_chunk({"actions": [action]})
            num_steps += 1
            ...
            chunk_obs = chunk_result.get("obs")
            if isinstance(chunk_obs, dict):
                total_reward += float(chunk_obs.get("reward", 0.0))
...
def _pick_action(
    ...
    ``param`` is either a runtime_obs dict (legacy path) or a TargetSessionHandle
    (new path via SessionRunner)."""
    action = action_plan[step_idx].copy()

    if action.get("type") == "move":
        target_type = action.get("params", {}).get("target")
        if target_type:
            if hasattr(param, "observe"):
                obs_data = param.observe().data
            else:
                obs_data = param
            entity = _find_nearest_entity(obs_data, target_type)
            if entity is None:
