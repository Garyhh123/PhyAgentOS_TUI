...
class StarDojo(gym.Env):
    # 阅读重点：StarDojo 是 benchmark 的 Gym 环境封装。
    # 它不直接操作游戏，而是通过 ActionProxy 调用 C# SMAPI Mod。
    def __init__(
            self, port: int = 5000,
            save_index: int = 0,
            new_game: bool = True,
    ...
    # 阅读重点：reset 会连接游戏、加载指定存档，并初始化 mmap reader。
    # 如果要做 PhyAgentOS episode reset，可以从这里和 InitTaskProxy 借鉴。
    def reset(
            self,
            seed = 42,
            options = None
    ...
    def _get_obs(self, is_rl = False) -> dict:
        # 阅读重点：observation 主入口。ActionProxy.observe() -> C# observe_v2 -> JSON。
        # 这里继续解码截图、整理 Player.Position，并交给 obs_preprocess 压缩成可读字段。
        # update state
        before = time.time()
    ...
    # 阅读重点：这里把 C# 原始 GameData 转成 LLM/evaluator 更容易用的 dict。
    # 它会保存截图、映射物品 id、压缩 surroundings、并补充小写常用字段。
    ...
    def step(self, action: list[int]):
        # 阅读重点：Gym/RL step 先把离散 action 解码成 ActionProxy 调用，再拉取下一帧 observation。
