...
class StarDojoLLM(StarDojo):
    ...
    def _process_obs(self, obs : dict):
        # 阅读重点：批量评测里给原 StarDojo agent 看的 obs 会先做 prompt 友好化处理；
        # 但 task.evaluate 使用的是 step 末尾重新取到的 raw/processed obs。
    ...
    def step(self, autoAction=None):
        # 阅读重点：原版单步闭环是 observe -> planning -> execute_actions -> observe -> task.evaluate。
        # 我们的 Track A + bridge benchmark 接入也应保持这个节奏，只是 planning/execute 来源不同。
...
def run_stardojo_batch(
    ...
    # 阅读重点：原始批量 benchmark runner。
    # 它遍历 tasks_params，为每个 yaml case 创建 task/env/agent，然后循环到成功或 max_turn_count。
    logging.basicConfig(
        filename='app.log',
        filemode='a',
