# 阅读重点：这些 helper 是 evaluator 的通用计数器。
# 多数任务不是看最终状态文本，而是比较 raw obs 中 inventory/farm/Progression 等字段的数量变化。
...
def surrounding_check(check_list: list, surrounding: list, last_surrounding: list, is_remove: bool, type: str) -> int:
    # 用前后两帧 surroundingsdata 的同一 tile 对比，判断物体/地形是否新增或消失。
    number = 0
    if is_remove:
...
def count_check(check_list: list, data_list: list, last_data_list: list, is_remove: bool, type: str) -> int:
    # evaluator 常用的差分入口：先分别 count 当前/上一帧，再取正向变化量。
