...
def load_task(type: str, id: int) -> base.TaskBase:
    # 阅读重点：benchmark case 的入口。type 对应 task_suite 下的 yaml 文件，
    # id 是 yaml 顺序中的第几个 case，返回值是带 init/evaluate 逻辑的 Task 对象。
    filename = type + ".yaml"
    ...
    task = None
    # yaml 只描述 case 参数；真正的成功判定逻辑由下面这些 Task 子类承接。
    ...
    elif type == "combat" or type == "combat_lite":
