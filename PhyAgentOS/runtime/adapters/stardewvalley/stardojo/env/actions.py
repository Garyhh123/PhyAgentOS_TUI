...
class ActionProxy:
    # 阅读重点：ActionProxy 是 Python 侧到 SMAPI Mod 的动作桥。
    # 上层调用普通 Python 方法；这里把它们编码成 method%arg1%arg2 字符串，
    # 再通过 127.0.0.1:{port} 发给 C# ModEntry 里的 TCP server。
    ...
    def _post_message(self, message: str, print_message: bool = True) -> str:
        ...
        start_time = time.time()
        try:
            # 阅读重点：StarDojo 的动作协议不是 HTTP，而是本地 TCP 文本协议。
            # C# 端以 <EOF> 作为普通文本响应的结束标记。
            host = '127.0.0.1'
            port = self.port
            ...
            if is_observe:
                # 阅读重点：旧版 observe 走 mmap + CBOR；当前 observe() 默认用 observe_v2。
                # 因此大多数阅读场景看 JSON 文本返回路径即可。
            ...
            while True:
                ...
                eof_tail = (eof_tail + data)[-len(eof_marker):]

                if eof_tail == eof_marker:
                    break

            full_response_bytes = b"".join(response)

            if full_response_bytes.endswith(eof_marker):
                full_response_bytes = full_response_bytes[:-len(eof_marker)]

            full_response = full_response_bytes.decode("utf-8", errors="replace")
    ...
    def move(self, x: int, y: int) -> bool:
        # 阅读重点：这里的 move 是“相对移动”，不是绝对地图坐标。
        # C# 侧会把相对偏移加到 Game1.player.TilePoint，再用 PathFindController 自动寻路。
    ...
    def use(self, direction: int) -> None:
        ...
        '''
        # 阅读重点：use/interact 都先 turn，再发无参数命令。
        # 因此上层只需要表达“朝哪个方向使用”，不要额外重复 turn。
        self.turn(direction)
        message = f"use"
        self._post_message(message)
    ...
    def interact(self, direction: int) -> None:
        ...
        direction: 0: up, 1: right, 2: down, 3: left
        '''
        # 阅读重点：interact 更像 Stardew 的右键/动作键，常用于开门、NPC、箱子、
        # 商店柜台、收获和部分播种逻辑。
        self.turn(direction)
        message = f"interact"
        self._post_message(message)
    ...
    def choose_option(self, option_index: int, quantity: int = None, direction: int = None) -> None:
        # 阅读重点：菜单动作统一走 choose_option。option_index=0 表示关闭菜单；
        # direction=0 表示 in/buy/take，direction=1 表示 out/sell/put。
        if quantity is None:
            quantity = 0
    ...
    def observe(self) -> str:
        # 阅读重点：当前主路径是 observe_v2，C# 返回完整 JSON 字符串。
        # Python 的 StarDojo._get_obs() 会继续解码截图并做字段预处理。
...
def convert_discrete_into_commands(action: list[int], action_proxy: ActionProxy, is_RL = False) -> str:
    ...
    # 阅读重点：这是 Gym/RL 的 MultiDiscrete action 解码器。
    # LLM runner 通常不走这条，而是走 basic_skills.py -> ActionProxy。
    # action list
