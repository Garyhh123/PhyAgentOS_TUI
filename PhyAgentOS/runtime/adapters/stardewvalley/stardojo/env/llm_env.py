...
class SkillExecutor:
    # 阅读重点：SkillExecutor 会动态加载 basic_skills.py，
    # 并把当前环境的 ActionProxy 注入到 skill module 的全局变量 actionproxy。
    def __init__(
        self,
        actionproxy: Any,
...
class StarDojoLLM(StarDojo):
    # 阅读重点：LLM runner 版本的环境。它继承 StarDojo 的 observe/action bridge，
    # 但 step() 不接收离散动作，而是让 agent 规划 skill call 再执行。
    ...
    def _process_obs(self, obs : dict):
        # 阅读重点：把结构化 observation 转成 prompt 友好格式：
        # surroundings 变相对坐标文本，inventory 变 slot_index 文本，time 变 AM/PM。
    ...
    def step(self, autoAction=None):
        # 阅读重点：LLM 闭环入口。先获取 observation，再暂停游戏做规划，
        # 然后通过 GameManager.execute_actions 执行 basic_skills.py 中注册的动作。
