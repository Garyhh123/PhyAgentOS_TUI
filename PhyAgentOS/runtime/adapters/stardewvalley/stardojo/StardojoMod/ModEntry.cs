...
namespace observeSpaceTest
{
    public class ModEntry : Mod
    {
        ...
        public override void Entry(IModHelper helper)
        {
            ...
            // 阅读重点：SMAPI Mod 在游戏进程内启动本地 TCP server，
            // Python ActionProxy 发送的所有 action/observe 命令都会进入这里。
        ...
        // 阅读重点：TCP server 入口，监听 127.0.0.1:{port}。
        // 每个 client message 会交给 HandleClientAsync，再切回游戏主线程执行。
        ...
        // 阅读重点：旧版二进制 observation 使用 mmap 返回 CBOR bytes；
        // 当前 Python 默认 observe_v2 走 JSON 文本，可以先跳过这条路径。
        ...
        // 阅读重点：读取 Python 发来的 method%arg1%arg2 字符串，
        // 执行后普通返回会拼上 <EOF>，Python 侧据此判断响应结束。
        ...
        // 阅读重点：动作执行前的安全等待。observe/pause/resume 等会跳过；
        // 普通动作会等待玩家不在工具动画、武器动画、fade、暂停等状态。
...
//                var canExit = !paused && !animating && !passingOut;
        ...
        // 阅读重点：把网络线程收到的命令排到下一次 GameLoop.UpdateTicked，
        // 确保真正修改 Game1 状态的代码在游戏主线程运行。
        ...
        // 阅读重点：命令路由核心。methodName 优先匹配 ActionsAPI，
        // 匹配不到再找 InitTaskAPI，然后用反射 Invoke。
