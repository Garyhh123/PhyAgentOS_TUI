...
namespace ActionSpace.actions
{
    ...
    {
        // 阅读重点：Actions 是真正触碰 Stardew Valley 内部状态的实现层。
        // ActionsAPI 只做参数转换和路由，最终多数逻辑会落到这里。
        ...
        // 阅读重点：interact 根据玩家朝向计算前方 tile/屏幕坐标，
        // 并处理 shipping bin、播种、箱子、家具、NPC/门等右键交互。
        ...
        // 阅读重点：use 的核心分派。当前物品是武器、可食用/可放置物、普通物品、工具时走不同路径。
        ...
        // Auto-pathing method
        // 阅读重点：移动不是键盘逐步模拟，而是 Stardew 自带 PathFindController 自动寻路。
        // 完成、warp 或菜单变化都会触发 onComplete。
        ...
        // 阅读重点：这是 C# 原始 observation 的顶层结构。
        // Python _get_obs 会把它 JSON 反序列化后进一步预处理。
        ...
        // 阅读重点：当前 Python observe() 使用的导出函数，返回 JSON 文本。
        // 旧版 ExportGameData 返回 CBOR bytes，可先作为历史路径理解。
        ...
        // 阅读重点：observation 采集总入口。这里汇总玩家、NPC、时间、农场、菜单、截图和周围 tile。
        ...
        // 阅读重点：symbolic perception 的基本单元。
        // 单个 tile 会导出物体、地形、建筑、作物、碎片、家具、出口、NPC 和可放置信息。
        ...
        // 阅读重点：以玩家 tile 为中心采样周围方形区域。
        // 这部分会成为 Python observation 中的 surroundingsdata / surroundings。
        ...
        // 阅读重点：把当前 UI 菜单转成结构化 observation。
        // choose_option 后续能否购物/对话/操作箱子，主要看这里识别出的菜单类型。
