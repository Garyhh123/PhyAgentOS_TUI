...
namespace ActionSpace.actions
{
	...
	{
        // 阅读重点：ActionsAPI 是 Python 可见的 C# 门面层。
        // Python 命令名必须和这里的 public static 方法名一致。
        ...
        // 阅读重点：Python ActionProxy.move() 实际调用 move_relative。
        // 这里把相对偏移转成目标 tile，再交给 Actions.StartAutoPathing。
        ...
        // 阅读重点：use 不带方向，方向已由 Python 侧先 turn 完成。
        // 具体物品/工具分派在 Actions.useWithAnim 中。
        ...
        // 阅读重点：interact 不带方向，方向已由 Python 侧先 turn 完成。
        // 它会进入右键/动作键相关逻辑。
        ...
        // 阅读重点：菜单动作分发器。根据当前 activeClickableMenu 决定是购物、卖出、对话还是箱子。
        // index<=0 关闭菜单；否则这里会把 LLM 的 1-based index 转成内部 0-based。
        ...
        // 阅读重点：当前 observation 主入口，返回 JSON 字符串给 Python。
        // size 控制玩家周围 tile 的观测半径。
