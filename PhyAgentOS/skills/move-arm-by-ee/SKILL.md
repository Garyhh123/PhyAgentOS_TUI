...
For a user-level robot task, create one `forge_task_create` aggregate, pass its `task_id`
to every Query and Action in the plan, reconcile all Actions to terminal state, and call
`forge_task_finalize`. If recovery verification requests replanning, append a revision with
`forge_task_begin_revision` on the same task. Calls without `task_id` are diagnostic or
operator calls only; they use the same Tool API but do not count toward task verification.
