...
class SettingsScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        yield AppHeader()
        yield NavBar("settings")
        with Vertical(id="settings-wrap"):
            yield SectionTitle("Settings")
            yield Label("Ctrl+K 命令 · Alt+Left/Right 切换页面 · Esc 返回聊天", classes="hint")
    ...
    def on_mount(self) -> None:
        ...
        self.call_after_refresh(self._finish_loading_settings)

    def _finish_loading_settings(self) -> None:
    ...
    def on_select_changed(self, event: Select.Changed) -> None:
        ...
        if event.select.id != "setting-theme":
            return
        if self._loading_settings or not isinstance(event.value, str):
            return
        if event.value not in THEME_LABELS:
            return

        theme_name = event.value
        config = self.app.config
        if get_theme_name(config.tui.theme) == theme_name and self.app.theme == theme_name:
            return
    ...
    def _save_settings(self) -> None:
        ...
        theme_value = self.query_one("#setting-theme", Select).value
        if isinstance(theme_value, str) and theme_value in THEME_LABELS:
            config.tui.theme = theme_value
